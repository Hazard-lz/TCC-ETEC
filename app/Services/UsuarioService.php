...
class UsuarioService extends BaseService {
    ...
    public function registrarUsuario($nome, $email, $senha, $tipo = 'comum', $telefone = null) {
        ...
        if ($idNovoUsuario) {
            ...
            $this->usuarioModel->salvarCodigo($idNovoUsuario, $codigo);
    ...
    public function autenticar($email, $senha) {
        ...
        if (isset($usuario['email_verificado']) && $usuario['email_verificado'] == 0) {
            ...
            $_SESSION['email_verificacao'] = $usuario['email']; // Guarda o e-mail na sessão
            return $this->erro('Você precisa verificar o seu e-mail. <a href="'.BASE_URL.'/verificar-email" style="color: #8b5cf6; font-weight: bold; text-decoration: none;">Clique aqui para inserir o código.</a>');
        }
    ...
    public function solicitarRecuperacaoSenha($email) {
        ...
        $codigo = mt_rand(100000, 999999); 
        $expiracao = date('Y-m-d H:i:s', strtotime('+30 minutes')); // Expira em 30 min

        // Chama o model atualizado
        $this->usuarioModel->salvarCodigo($usuario['id_usuario'], $codigo, $expiracao);
